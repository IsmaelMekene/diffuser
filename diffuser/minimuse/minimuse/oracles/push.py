...
from diffuser.minimuse.minimuse.core.constants import MAX_TOOL_VELOCITY, CONTROLLER_DT
from diffuser.minimuse.minimuse.core.utils import muj_to_std_quat, quat_to_euler
from diffuser.minimuse.minimuse.oracles.utils import rotate, theta_to_rotation2d, get_theta_from_vector
...
class PushOracle:
    ...
    def get_action(self, obs):
        ...
        max_step_distance = MAX_TOOL_VELOCITY[0] * CONTROLLER_DT
        ...
        action = dict(xy_linear_velocity=xy_linear_velocity)
        return action

    def _move_to_preblock(self, obs):
        block_xy = obs[f"{self._obj_name}_pos"][:2]
        goal_xy = obs[f"{self._goal_name}_pos"][:2]
        tool_xy = obs["tool_pos"][:2]

        block_to_goal_xy = goal_xy - block_xy
        block_to_goal_xy_dir = block_to_goal_xy / np.linalg.norm(block_to_goal_xy)

        # Go 3 cm away from the block, on the line between the block and target.
        preblock_xy = block_xy - block_to_goal_xy_dir * 0.03
        relative_xy = preblock_xy - tool_xy
    ...
    def _return_to_first_preblock(self, obs):
        ...
        relative_to_origin = self.first_preblock - tool_xy
    ...
    def _return_to_origin(self, obs):
        ...
        tool_xy = obs["tool_pos"][:2]
        relative_to_origin = self.origin - tool_xy
        diff = np.linalg.norm(relative_to_origin)
        if diff < 0.001:
            self.state = "move_to_preblock"
        return relative_to_origin
    ...
    def _move_to_block(self, obs):
        ...
        relative_xy = next_to_block_xy - tool_xy
        diff = np.linalg.norm(relative_xy)
        if diff < 0.001:
    ...
    def _push_block(self, obs):
        ...
        touching_block_xy = block_xy - block_to_goal_xy_dir * 0.001
    ...
    def _orient_block(self, obs, direction="right"):
        ...
        block_to_tool_xy = tool_xy - block_xy
        block_to_tool_xy_dir = block_to_tool_xy / np.linalg.norm(block_to_tool_xy)
        ...
        block_to_tool_xy_dir = rotate(theta_rotation, block_to_tool_xy_dir)
        block_to_tool_xy = block_to_tool_xy_dir * self.orient_circle_diameter
        push_left_spot_xy = block_xy + block_to_tool_xy
        relative_xy = push_left_spot_xy - tool_xy
